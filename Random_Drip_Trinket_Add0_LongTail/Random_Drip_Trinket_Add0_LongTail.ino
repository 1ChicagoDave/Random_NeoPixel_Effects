...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(42, PIN, NEO_GRB + NEO_KHZ800); //first number controls the amount of pixels you have (add 4 so the drip falls off the edge)

int bright = 1;    //select 1 thru 10
int wait = 20;    //speed of leds
...
void colorWipe(int r, int g, int b, int wait) {
  for(int i=0; i<strip.numPixels(); i++) {
    ...
    int randr0 = r -(6*bright);
    if (randr0 < 1) randr0 = 10 + bright;
    int randg0 = g -(6*bright); 
    if (randg0 < 1) randg0 = 10 + bright;
    int randb0 = b -(6*bright);
    if (randb0 < 1) randb0 = 10 + bright;
    ...
    strip.setPixelColor(i-0, randr0, randg0, randb0);
    strip.setPixelColor(i-1, randr1, randg1, randb1);
    strip.setPixelColor(i-2, randr2, randg2, randb2);
    strip.setPixelColor(i-3, randr3, randg3, randb3);     
    strip.setPixelColor(i-3, randr3, randg3, randb3);
    ...
    strip.show();
    delay(wait);
    strip.setPixelColor(i-6, strip.Color(0,0,0)); // make sure the i-number is the amount of the trail
    strip.show();
    delay(wait);   
  }  
}
